var app = new Vue({
  ...
  methods: {
    ...
    deleteBahanAjar() {
      ...
      this.dataBahanAjar.stok = this.dataBahanAjar.stok.filter((item) => item.kode !== kodeToDelete);
      this.closePopup();
    },

    // DO
    createNewDo() {
      const lastTracking = Object.keys(this.dataBahanAjar.tracking[this.dataBahanAjar.tracking.length - 1])[0] ;
    ...
    saveModalDo() {
      if(!this.modalDo.nim || !this.modalDo.nama || !this.modalDo.ekspedisi || !this.modalDo.paket || !this.modalDo.tanggalKirim){
      ...
      this.dataBahanAjar.tracking.push({[this.modalDo.nomorDeliveryOrder]: newData});
